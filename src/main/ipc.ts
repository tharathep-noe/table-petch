...
export function registerIpc(): void {
  ...
  ipcMain.handle(CH.saveConnection, (_e, input: ConnectionInput) => store.saveConnection(input))
  ipcMain.handle(CH.deleteConnection, (_e, id: string) => store.deleteConnection(id))
  ipcMain.handle(CH.testConnection, (_e, input: ConnectionInput) => mgr.testConnection(input))

  ipcMain.handle(CH.connect, (_e, id: string) => mgr.connect(id))
  ipcMain.handle(CH.disconnect, (_e, id: string) => mgr.disconnect(id))
  ipcMain.handle(CH.switchDatabase, (_e, id: string, database: string) => mgr.connect(id, database))

  ipcMain.handle(CH.listSchema, (_e, id: string) => listSchema(id))
  ipcMain.handle(CH.loadRows, (_e, req: LoadRowsRequest) => loadRows(req))
  ipcMain.handle(CH.runQuery, (_e, id: string, sql: string) => runQuery(id, sql))
